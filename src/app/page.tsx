...
// Mock Data representativa de nuestra DB
        ...
        <section className="listings-grid">
          {filteredListings.length > 0 ? (
            filteredListings.map((listing) => (
              <Link href={`/listings/${listing.id}`} key={listing.id} className="glass-panel" style={{ textDecoration: 'none', padding: 0, display: 'flex', flexDirection: 'column', transition: 'all 0.2sease-in-out', overflow: 'hidden' }}>
                {listing.maquinaria && listing.maquinaria.fotoUrl ? (
                  <div style={{ backgroundColor: '#fff', padding: '1rem', display: 'flex', justifyContent: 'center', alignItems: 'center', height: '220px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    <img
